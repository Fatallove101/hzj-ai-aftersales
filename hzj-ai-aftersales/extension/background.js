...
// ⚠️ 必须给够时间。踩过的坑：原来是 30000（30 秒），
// 但接上千帆后**一次分析要 33~47 秒**（实测 38.2 秒：
// translate 5.6s + generate 27.2s，模型本身的延迟）。
// 结果每次点「换一批」都在 30 秒被 AbortController 掐断，
// 界面显示"生成失败（连接正常）" —— 看起来像服务端出错，其实是前端超时。
// 现在给 150 秒，留足余量。
//
// 顺带说明：本地服务是**单线程**的，一次只处理一个连接。
// 生成话术那几十秒里，它无法响应其他请求（比如别的标签页的健康检查）。
// 所以这里也不能太短 —— 否则在生成过程中打开另一个页面的扩展，
// 健康检查会超时，误报"未连接"。
